...
(function() {
    ...
    let secondsLeft = parseInt(timerEl.dataset.secondsLeft, 10);
    if (isNaN(secondsLeft)) return;

    let enforced = timerEl.dataset.enforced === 'true';
    ...
    function render() {
        valueEl.textContent = formatTime(Math.max(0, secondsLeft));
        timerEl.classList.toggle('session-timer-warning', secondsLeft <= 60);
    }
    ...
    function startTicking() {
        ...
        tickInterval = setInterval(() => {
            ...
            if (secondsLeft <= 0) {
                ...
                stopTicking();
                if (enforced) {
                    // /logout безусловно завершает сессию (flask-login
                    // logout_user + очистка куки) вне зависимости от того,
                    // защищена ли текущая страница session_required.
    ...
    const isStatusUrl = (url) => typeof url === 'string' && url.indexOf('/api/session-status') !== -1;

    const originalFetch = window.fetch;
    if (originalFetch) {
        window.fetch = function(input, init) {
            const url = typeof input === 'string' ? input : (input && input.url);
            const skip = isStatusUrl(url);
            return originalFetch.apply(this, arguments).then((res) => {
                if (!skip) scheduleResync();
                return res;
            }, (err) => {
                if (!skip) scheduleResync();
                throw err;
    ...
    if (OriginalXHR) {
        ...
        OriginalXHR.prototype.open = function(method, url) {
            this.__isSessionStatusUrl = isStatusUrl(url);
            return originalOpen.apply(this, arguments);
        };
        const originalSend = OriginalXHR.prototype.send;
        OriginalXHR.prototype.send = function() {
            this.addEventListener('loadend', () => {
                if (!this.__isSessionStatusUrl) scheduleResync();
            });
